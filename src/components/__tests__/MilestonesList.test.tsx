...
describe('MilestonesList', () => {
  ...
  describe('scroll region labelling', () => {
    ...
    it('count span reflects a single-item list', () => {
      const { container } = render(<MilestonesList milestones={[SAMPLE[0]]} />);
      expect(container.querySelector('#milestones-count')).toHaveTextContent(
        '1 total',
      );
    });
  ...
  it('renders an accessible warning for milestone currencies that differ from the contract', () => {
    ...
    expect(alert).toHaveTextContent(
      '2 milestones use EUR, GBP instead of USD.',
    );
  ...
  describe('due-soon reminder banner', () => {
    ...
    it('renders banner with correct pluralization for 1 due-soon milestone', () => {
      ...
      render(<MilestonesList milestones={milestones} />);
      expect(
        screen.getByText('1 milestone is due within 7 days'),
      ).toBeInTheDocument();
      expect(
        screen.getByRole('link', { name: 'Due Soon Milestone' }),
      ).toHaveAttribute('href', '#milestone-1');
    });
    ...
    it('renders banner with correct pluralization for multiple due-soon milestones', () => {
      ...
      render(<MilestonesList milestones={milestones} />);
      expect(
        screen.getByText('2 milestones are due within 7 days'),
      ).toBeInTheDocument();
      expect(screen.getByRole('link', { name: 'Milestone A' })).toHaveAttribute(
        'href',
        '#milestone-1',
      );
      expect(screen.getByRole('link', { name: 'Milestone B' })).toHaveAttribute(
        'href',
        '#milestone-2',
      );
    });
